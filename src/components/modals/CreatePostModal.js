...
const CreatePostModal = ({ isOpen, onClose, onPostCreated, groupId }) => {
  ...
  const handleUploadSong = async () => {
    ...
    try {
      ...
      // Use songService.uploadSong with query parameters as requested
      ...
      if (msg.toLowerCase().includes("duplicate") || msg.toLowerCase().includes("exists")) {
        alert("Warning: This song might already exist in the system (duplicate name and artist).");
      } else {
  ...
  const handleSubmitPost = async () => {
    ...
    try {
      ...
      // Clear this draft if posted successfully (if draft has same song ID)
            ...
            <div className="step-2-content animate-in slide-in-from-right-4 duration-300">
              <div className="flex gap-4 mb-4">
                <div className="shrink-0">
                  <label className="text-xs font-bold uppercase tracking-wider opacity-60 mb-1.5 block">Cover Image</label>
                  <div
                ...
                <div className="flex-1 flex flex-col">
                  <div className="flex justify-between items-center mb-1.5">
                    <label className="text-xs font-bold uppercase tracking-wider opacity-60 block">Your Caption</label>
                    <select
                      value={visibility}
                      onChange={(e) => setVisibility(e.target.value)}
                      className="text-xs font-bold py-1.5 px-3 bg-indigo-50 dark:bg-slate-800 text-indigo-600 dark:text-blue-400 border border-indigo-100 dark:border-slate-700 rounded-full outline-none hover:bg-indigo-100 transition-colors cursor-pointer shadow-sm"
                    >
